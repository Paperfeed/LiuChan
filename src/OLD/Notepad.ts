...
export class Notepad {
  ...
  saveToStorage() {
    ...
    chrome.runtime.sendMessage({ query: notepad, type: 'notepad' })
  }
  ...
  checkPageBoundary() {
    ...
    if (overlay.offsetWidth > d.clientWidth)
      textarea.style.width = d.clientWidth - offsetX + 'px'
    if (overlay.offsetHeight > d.clientHeight)
      textarea.style.height = d.clientHeight - offsetY + 'px'
  ...
  lostFocus(e) {
    if (
      e.srcElement.offsetParent !== this.elements[0] &&
      e.srcElement !== this.elements[0] &&
      !this.isPinned
    ) {
      this.closeOverlay()
    }
